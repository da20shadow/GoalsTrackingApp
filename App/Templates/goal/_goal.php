<?php
use App\Models\goals\GoalDTO; /** @var GoalDTO $data */
...
$tasks = $goalService->getTasksByGoalID($data->getGoalID());
...
<div class="container">
    ...
    <button class="btn btn-lg border-2 shadow-sm" onclick="history.back()">
    ...
    <h1 class="text-center text-secondary fw-bold my-4">GOAL: <?= $data->getGoalTitle() ?></h1>

    <p class="text-center my-3 fst-italic"><svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" class="bi bi-calendar-date" viewBox="0 0 16 16">
        ...
        </svg> Mar 18, 2022 - <?= $dueDate ?> <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" class="bi bi-flag" viewBox="0 0 16 16">
...
<!--    Goal Description-->
    <div class="my-3 shadow-sm border border-2 rounded border-secondary">
        ...
        <textarea class="form-control" id="desc" rows="6"><?= $data->getGoalDescription() ?></textarea>
...
<!--    Target Per Day %-->
  ...
  <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z"/>
    ...
    <div class="row mt-5">
        ...
        <div class="col-6 fw-bold fs-6">Task</div>
        <div class="col-1 fw-bold fs-6">Due Date</div>
        <div class="col-1 fw-bold fs-6">Priority</div>
        <div class="col-2 fw-bold fs-6">Status</div>
        <div class="col-2 fw-bold fs-6">Progress</div>
    ...
    <?php foreach ($tasks as $task): ?>
        ...
        <div class="col-6">
            <a class="text-dark" href="task.php?id=<?= $task->getTaskID() ?>">
                <?= $task->getTaskTitle() ?>
            </a>
            <span class="badge bg-primary rounded-pill" title="Subtasks">11</span>
        ...
        <div class="col-1 small">
            <?php echo (date("M d, Y",strtotime($task->getDueDate()))); ?>
        </div>
<!--            Task Priority-->
        <div class="col-1">
            <div class="btn-group">
...
<!--            Task Status-->
        <div class="col-2">
            ...
            <div class="btn-group">
                <button class="btn" type="button" id="dropdownStatusButton" data-bs-toggle="dropdown" aria-expanded="false">
                    IN REVIEW
                </button>
                <ul class="dropdown-menu" aria-labelledby="dropdownStatusButton">
                    <li onclick="changeStatusToDo()" class="dropdown-item">
                        TO DO
                    </li>
                    <li onclick="changeStatusToProgress()" class="dropdown-item">
                        IN PROGRESS
                    </li>
                    <li onclick="changeStatusToReview()" class="dropdown-item">
                        IN REVIEW
                    </li>
                    <li onclick="changeStatusToCompleted()" class="dropdown-item">
                            COMPLETED
                    </li>
                </ul>
            </div>
    ...
    <div class="row">
        ...
        <div class="col-6">
            <input class="form-check-input" style="font-size: 19px" type="checkbox">
            Task Title IS Here Now
        </div>
        <div class="col-1">May 26, 2022</div>
        <div class="col-1">
            ...
            <div class="btn-group">
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    ...
                    <li><a onclick="changeStatusTwo()" class="dropdown-item">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="orange" class="bi bi-flag-fill" viewBox="0 0 16 16">
                                <path d="M14.778.085A.5.5 0 0 1 15 .5V8a.5.5 0 0 1-.314.464L14.5 8l.186.464-.003.001-.006.003-.023.009a12.435 12.435 0 0 1-.397.15c-.264.095-.631.223-1.047.35-.816.252-1.879.523-2.71.523-.847 0-1.548-.28-2.158-.525l-.028-.01C7.68 8.71 7.14 8.5 6.5 8.5c-.7 0-1.638.23-2.437.477A19.626 19.626 0 0 0 3 9.342V15.5a.5.5 0 0 1-1 0V.5a.5.5 0 0 1 1 0v.282c.226-.079.496-.17.79-.26C4.606.272 5.67 0 6.5 0c.84 0 1.524.277 2.121.519l.043.018C9.286.788 9.828 1 10.5 1c.7 0 1.638-.23 2.437-.477a19.587 19.587 0 0 0 1.349-.476l.019-.007.004-.002h.001"/>
                            </svg> High
                        </a>
                    ...
                    <li><a onclick="changeStatus()" class="dropdown-item">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="gray" class="bi bi-flag-fill" viewBox="0 0 16 16">
                                <path d="M14.778.085A.5.5 0 0 1 15 .5V8a.5.5 0 0 1-.314.464L14.5 8l.186.464-.003.001-.006.003-.023.009a12.435 12.435 0 0 1-.397.15c-.264.095-.631.223-1.047.35-.816.252-1.879.523-2.71.523-.847 0-1.548-.28-2.158-.525l-.028-.01C7.68 8.71 7.14 8.5 6.5 8.5c-.7 0-1.638.23-2.437.477A19.626 19.626 0 0 0 3 9.342V15.5a.5.5 0 0 1-1 0V.5a.5.5 0 0 1 1 0v.282c.226-.079.496-.17.79-.26C4.606.272 5.67 0 6.5 0c.84 0 1.524.277 2.121.519l.043.018C9.286.788 9.828 1 10.5 1c.7 0 1.638-.23 2.437-.477a19.587 19.587 0 0 0 1.349-.476l.019-.007.004-.002h.001"/>
                            </svg> Low
                        </a>
        ...
        <script>
            function changeStatusToCompleted(){
                document.getElementById('dropdownStatusButton').innerHTML = "COMPLETED!";
            }
            function changeStatusToReview(){
                document.getElementById('dropdownStatusButton').innerHTML = "IN REVIEW!";
            }
            function changeStatusToProgress(){
                document.getElementById('dropdownStatusButton').innerHTML = "IN PROGRESS!";
            }
            function changeStatusToDo(){
                document.getElementById('dropdownStatusButton')
                    .innerHTML = "TO DO!";
            }
        </script>
        <div class="col-2" id="test">In Progress</div>
        <div class="col-2">
            <div class="progress w-75" style="height: 25px;">
                <div id="progressBar" class="progress-bar bg-success"
    ...
    <div class="row">
        ...
        <div class="col-6">Task Title IS Here Now</div>
        <div class="col-1">May 6, 2022</div>
        <div class="col-1">
            <div class="btn-group">
                <button class="btn" type="button" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="orange" class="bi bi-flag-fill" viewBox="0 0 16 16">
        ...
        <div class="col-2">In Progress</div>
        <div class="col-2">
            <div class="progress w-75" style="height: 25px;">
                <div id="progressBar" class="progress-bar bg-success"
    ...
    <button class="btn shadow-sm border d-flex justify-content-center m-auto my-4">
        ...
        </svg> Completed <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-arrow-down" viewBox="0 0 16 16">
            <path fill-rule="evenodd" d="M8 1a.5.5 0 0 1 .5.5v11.793l3.146-3.147a.5.5 0 0 1 .708.708l-4 4a.5.5 0 0 1-.708 0l-4-4a.5.5 0 0 1 .708-.708L7.5 13.293V1.5A.5.5 0 0 1 8 1z"/>
        </svg></button>

    <div class="row bg-light">

        <div class="col-6 text-decoration-line-through">Task Title IS Here Now</div>
        <div class="col-1">May 1, 2022</div>
        <div class="col-1"><svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="gray" class="bi bi-flag" viewBox="0 0 16 16">
                <path d="M14.778.085A.5.5 0 0 1 15 .5V8a.5.5 0 0 1-.314.464L14.5 8l.186.464-.003.001-.006.003-.023.009a12.435 12.435 0 0 1-.397.15c-.264.095-.631.223-1.047.35-.816.252-1.879.523-2.71.523-.847 0-1.548-.28-2.158-.525l-.028-.01C7.68 8.71 7.14 8.5 6.5 8.5c-.7 0-1.638.23-2.437.477A19.626 19.626 0 0 0 3 9.342V15.5a.5.5 0 0 1-1 0V.5a.5.5 0 0 1 1 0v.282c.226-.079.496-.17.79-.26C4.606.272 5.67 0 6.5 0c.84 0 1.524.277 2.121.519l.043.018C9.286.788 9.828 1 10.5 1c.7 0 1.638-.23 2.437-.477a19.587 19.587 0 0 0 1.349-.476l.019-.007.004-.002h.001M14 1.221c-.22.078-.48.167-.766.255-.81.252-1.872.523-2.734.523-.886 0-1.592-.286-2.203-.534l-.008-.003C7.662 1.21 7.139 1 6.5 1c-.669 0-1.606.229-2.415.478A21.294 21.294 0 0 0 3 1.845v6.433c.22-.078.48-.167.766-.255C4.576 7.77 5.638 7.5 6.5 7.5c.847 0 1.548.28 2.158.525l.028.01C9.32 8.29 9.86 8.5 10.5 8.5c.668 0 1.606-.229 2.415-.478A21.317 21.317 0 0 0 14 7.655V1.222z"/>
            </svg></div>
